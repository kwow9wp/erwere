...
function showGeoBlocked(reason) {
  ...
  if (reason === 'denied') msg.textContent = 'Разрешите доступ к местоположению в настройках Telegram, чтобы определить ваше положение.';
  ...
  else msg.textContent = 'Геолокация недоступна.';

  // Telegram MainButton for retry
  if (tg?.MainButton) {
    tg.MainButton.setText('📍 Определить местоположение');
    tg.MainButton.show();
